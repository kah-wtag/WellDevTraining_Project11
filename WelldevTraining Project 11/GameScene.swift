//
//  GameScene.swift
//  WelldevTraining Project 11
//
//  Created by Md. Kamrul Hasan on 30/7/25.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if objects.contains(deleteLabel) {
            deletingMode.toggle()
            editingMode = false
            return
        }
        ...
        if editingMode {
            ...
            if let tappedBox = objects.first(where: { $0.name == "box" }) {
                tappedBox.removeFromParent()
            }
        } else {
